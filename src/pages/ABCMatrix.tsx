...
import {
  ...
  type AnalysisRow, type ClassifiedRow, type SegmentKey,
...
export default function ABCMatrix() {
  ...
  const activeCatOverrides = useMemo(() => {
    ...
    for (const [cat, state] of Object.entries(catThresholds)) {
      if (state.on) result[cat] = { a: state.a, c: state.c };
    }
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-5 space-y-4">
          ...
          {categories.length > 0 && (
            <div className="border-t border-slate-100 pt-3">
              <button
                onClick={() => setShowCatThresholds(v => !v)}
                className="flex items-center gap-2 w-full text-left text-xs font-medium text-slate-600 hover:text-slate-800 transition-colors"
              >
                <ChevronDown className={`w-3.5 h-3.5 text-slate-400 transition-transform ${showCatThresholds ? 'rotate-0' : '-rotate-90'}`} />
                Soglie per categoria
              </button>
              {showCatThresholds && (
                <div className="mt-2 max-h-56 overflow-y-auto space-y-1.5 pr-1">
                  {categories.map(cat => {
                    const state = catThresholds[cat.category] ?? { on: false, a: thresholdA, c: thresholdC };
                    return (
                      <div key={cat.category} className="border border-slate-100 rounded-lg p-2 space-y-1.5">
                        <div className="flex items-center gap-2">
                          <input
                            type="checkbox"
                            checked={state.on}
                            onChange={e => setCatThresholds(prev => ({
                              ...prev,
                              [cat.category]: { ...state, on: e.target.checked },
                            }))}
                            className="rounded"
                          />
                          <span className="text-[11px] font-medium text-slate-700 truncate">{cat.category}</span>
                        </div>
                        {state.on && (
                          <div className="grid grid-cols-2 gap-1.5 ml-5">
                            <div>
                              <label className="text-[10px] text-slate-400">Soglia A%</label>
                              <input
                                type="number"
                                value={state.a}
                                onChange={e => setCatThresholds(prev => ({
                                  ...prev,
                                  [cat.category]: { ...state, a: +e.target.value },
                                }))}
                                className="w-full h-7 px-2 border border-slate-200 rounded text-xs bg-slate-50 focus:outline-none focus:border-blue-400"
                              />
                            </div>
                            <div>
                              <label className="text-[10px] text-slate-400">Soglia C%</label>
                              <input
                                type="number"
                                value={state.c}
                                onChange={e => setCatThresholds(prev => ({
                                  ...prev,
                                  [cat.category]: { ...state, c: +e.target.value },
                                }))}
                                className="w-full h-7 px-2 border border-slate-200 rounded text-xs bg-slate-50 focus:outline-none focus:border-blue-400"
                              />
                            </div>
